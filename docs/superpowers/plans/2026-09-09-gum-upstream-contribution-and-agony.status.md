...
| 5 | COMPLETE | main | `633ebf5` condições/motor, `83de6fb` SVGs, `c55ff66` documentação, `5c24f17` retomada testada, `ccf6be0` autocontrole na Agonia, `08562e4` migração V4 e `b21efcf` conclui as 11 condições restantes | passed | revisão humana; sem push |
...
Validação adicional: o sistema foi sincronizado por filesystem com a instalação local do Foundry, sem uso da UI para editar os arquivos, e o mundo `teste` foi recarregado. A migração mostrou as 19 condições incapacitantes e de dor com SVGs dedicados. Asfixia exibiu os vínculos `Condição — Asfixia` e `Deitado`; Agonia exibiu `Agonia — Incapacitação`, `Deitado` e a entrada de autocontrole em `check_dx,skill_dx,check_iq,skill_iq,self_control`. Os testes específicos de migração passaram em 6/6.
