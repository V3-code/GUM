...
export class TriggerSheet extends ItemSheet {
    ...
    // ✅ LÓGICA DOS BOTÕES DA PALETA (adaptada do ConditionBuilder)
    _onPaletteClick(event) {
        event.preventDefault();
        const action = event.currentTarget.dataset.action;
        const textarea = this.element.find('textarea[name="system.code"]')[0]; // Alvo é o campo "code"
        ...
        switch(action) {
            ...
            case 'open-attributes':
                ...
                break;
            case 'open-operators':
                this._openPicker("Selecionar Operador", GUM_DATA.operators, "", textarea);
                break;
            case 'open-connectors':
                const connectors = { " && ": "E (&&)", " || ": "OU (||)" };
                this._openPicker("Selecionar Conector", connectors, "", textarea);
                break;
        }
    }
